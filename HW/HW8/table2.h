...
namespace main_savitch_12B
{
    ...
    public:
        ...
        static const std::size_t TABLE_SIZE = 7;
        ...
        void displayHash()
        {
          ...
          {
            // std::cout << main_savitch_6B::list_length(data[i]);
            for(int j = 1; j <= main_savitch_6B::list_length((data[i])); j++)
            {
              std::cout << main_savitch_6B::linst_locate(data[i],j) -> data() << " ";
            }
            std::cout << std::endl;
          }
        }
